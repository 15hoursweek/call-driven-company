...
| ユーザーTODO | user_todo/{YYYYMMDD}_{状態}_{要点}.md | user_todo/ | 要人間対応の発生時（1TODO=1ファイル） | チャット（クラウド）・CodeCLI | 本人にしかできない作業（収録基準は下記）。状態は `PENDING`。消化したらファイルごと削除する |

## user_todo の収録基準

`user_todo/` は**本人の手が要る作業だけ**を置く場所。置いてよいのは次に当たるものだけとする:

- 認証情報・支払い情報の入力、二要素認証、CAPTCHA
- 本人性が要る同意・承認（審査提出、公開の最終承認、契約への同意）
- 本人しか持たない情報の提供、本人にしか下せない意思決定

これ以外は置かない。判定にあたって次の3点を必ず守る:

1. **試してから判定する。** 「AIには無理」を推測で決めない。実際に試して拒否・失敗を確認し、試した操作と結果をファイルに書く。試していないものは user_todo に出さない
2. **無人でできないだけのものは `ai_todo/` の `対話待ち` に置く。** 無人実行が止まる主因は権限分類器の拒否と画面ロックで、対話セッションのClaudeにはどちらも無い。これは本人の作業ではないので user_todo に出さない
3. **出す前に完了済みでないか確認する。** 関連する `ai_todo/` の実行記録と対象ドキュメントの反映状況を見て、済んでいる作業を本人に投げない

## 引き渡し（user_todo を作るときの必須手順）

ファイルを書いただけでは引き渡しは終わっていない。作成時に次まで行う:

- 本文の先頭に `引き渡し先: {本人が操作する画面のURL、またはアプリ名と到達手順}` を1行で書く。URLは実在を確認してから書く
- その画面をブラウザで開いた状態にする。Chrome操作（navigate・クリック・読み取り）は無人セッションでも使える
- 開いたタブは閉じず、どのタブを残したかを本文に書く。ネイティブアプリで無人のため開けない場合は、その旨を `引き渡し先:` の行に書き、次の対話セッションで開く

対話セッションは冒頭で `user_todo/` と `ai_todo/` の `対話待ち` を受け取る（`ops/session-handoff.sh` を SessionStart フックが実行する）。`対話待ち` はその場でClaudeが実行し、`user_todo/` は引き渡し先を開いてから本人に提示する。
...
- チャットの出力先は、`ai_todo/` への新規TODOファイル作成・`idea/` への新規IDEA_ファイル・`user_todo/` のファイル作成/削除（詳細は②）
- `user_todo/` は**人間のボールだけ**を置く場所。AI側の作業は `ai_todo/` で管理し、AIの作業を待つ理由でuser_todoにファイルを残さない
- チャットは指示を直接実行しない。理念・企画・原稿など純ドキュメントの修正も含め、実行が必要なことはすべて `ai_todo/` の1ファイルとして積み、CLIが実行する

### ② 正本への保存（チャット側の出口）

チャット側の出口は**TODO・IDEA・user_todoのファイル操作だけ**。それ以外のドキュメントは直接作成・更新せず、`ai_todo/` にTODOとして積んでCLIに委ねる。許可されたファイル操作はGitHub MCPコネクタで行う（コミットメッセージは内容の一行要約。保存・更新の前にファイル名・内容を確認する）。
...
2. **TODOは1ファイル1TODOで、状態をファイル名で管理する**（`{YYYYMMDD}_{状態}_{要点}.md`。日付は作成日で以後変更しない。日付が先頭のため、並べたとき古いものが上に来て滞留が見える）。状態変更はリネームで行う（日付部分は据え置き）。状態は記号ではなく文字で書き、定義はここが正本（他のファイルに写さない）:
   ...
   - `対話待ち` — 無人実行では通らないが、対話セッションのClaudeなら実行できるもの（権限分類器の拒否、画面ロックが要因の失敗）。定期実行の対象から外し、`user_todo/` は作らない。次の対話セッションでClaudeが実行する
   - `人間待ち` — 本人の手が要る対応が必要で止まったもの（収録基準は「user_todo の収録基準」が正本）。定期実行の対象から外し、`user_todo/` に対応依頼のファイルを作成し、同じ実行のうちに引き渡しまで行う（同件があれば重複作成しない）。再試行しても直らない失敗を繰り返さないこと
...
4. **`人間待ち` の解除は人間の対応後**: 打ち合わせでuser_todoの該当ファイルを消化した際、チャットが `ai_todo/` の該当ファイルの状態部分を `未着手` にリネームして戻す（またはユーザーが直接戻す）。戻ったTODOは次回の定期実行から通常どおり実行される。`対話待ち` は対話セッションのClaudeが実行し、完了すればファイルごと削除する（解除待ちにしない）
