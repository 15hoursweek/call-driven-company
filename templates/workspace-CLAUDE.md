...
- セッション冒頭に `docs/ops/sync-docs.sh` を実行して正本と同期し、続けて `docs/ops/session-handoff.sh` で未消化TODOを受け取る（`~/.claude/settings.json` の SessionStart フックに両方を並べて登録しておく）。`docs/` を編集したときも sync スクリプトで書き戻すこと
